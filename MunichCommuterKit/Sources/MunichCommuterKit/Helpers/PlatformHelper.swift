...
public struct PlatformHelper {
    public static func effectivePlatform(from platformProps: PlatformProperties?) -> String? {
        return platformProps?.platformName
            ?? platformProps?.plannedPlatformName
            ?? platformProps?.platform
    }

    public static func sortPlatforms(_ platforms: [String]) -> [String] {
